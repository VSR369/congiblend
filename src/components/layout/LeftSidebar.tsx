...
export const LeftSidebar = () => {
  ...
  return (
    <div className="w-80 h-screen overflow-y-auto p-4 space-y-6 glass-card border-r border-white/10">
      {/* User Profile Card */}
      <motion.div 
      ...
      <motion.div
        ...
        className="glass-card p-4 rounded-xl border border-white/10"
      >
        <h3 className="text-muted-foreground font-medium mb-4">Navigation</h3>
          ...
          {navigationItems.map((item, index) => (
            ...
            >
              <Link 
                to={item.url}
                className={`
                  flex items-center space-x-3 px-3 py-2 rounded-lg transition-all duration-200 hover:bg-primary/10 hover:text-primary
                  ${location.pathname === item.url ? 'bg-primary/10 text-primary border-r-2 border-primary' : 'text-foreground'}
                `}
              >
